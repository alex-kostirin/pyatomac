...
class _Method(xmlrpclib._Method):
    def __call__(self, *args, **kwargs):
        if _ldtp_debug:
            logger.debug('%s(%s)' % (self.__name, ', '.join(list(map(repr, args)) + ['%s=%s' % (k, repr(v)) for k, v in kwargs.items()])))
...
class Transport(xmlrpclib.Transport):
    ...
    def _spawn_daemon(self):
        ...
        if platform.mac_ver()[0] != '':
            ...
            self._daemon = os.spawnlp(os.P_NOWAIT, python_command,
                                      python_command, '-c', pycmd)
        else:
